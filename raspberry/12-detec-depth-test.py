...
start_dist = 0.3 / depth_scale # 30 сантиметров
...
def filter_contours(contours):
    for c in contours:
        ...
        if (w / h < 1.3 and w / h > 0.7) and (area > 1000 and area < 15000):
            return x, y, w, h, area
...
while True:
    ...
    img_black = 255*np.ones((cut_depth_image.shape[0], cut_depth_image.shape[1]), np.uint8)
    bgr_depth = np.where((cut_depth_image > end_dist) | (cut_depth_image <= start_dist),
                         0, img_black )
   
   
    edged_depth_image = cv2.Canny(bgr_depth, 100, 250, apertureSize = 5)
    # Получение изображения с выделенными краями объектов в кадре
    # edged_image = edge_detector(bgr_depth)
    # Извлечение контуров из кадра с выделенными границами и рисование в кадре всех контуров
    contours, _ = cv2.findContours(edged_depth_image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
