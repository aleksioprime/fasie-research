...
def find_template(contours, temp):
    for c in contours:
        ...
        c = cv2.approxPolyDP(c, 0.005 * peri, True)
        x, y, w, h = cv2.boundingRect(c)
        area = cv2.contourArea(c)
        for t in temp:
            match = cv2.matchShapes(t, c, 1, 0)
            if match < 0.001 and (area > 3500 and area < 9000) and (w / h < 1.3 and w / h > 0.7):
                return c, t
        return None
...
while True:
    ...
    data_cnt = find_template(sorted_contours, templates)  
    if data_cnt is not None:
        cnt, tmp = data_cnt
        cv2.drawContours(cut_image_test, [cnt], -1, (0,0,255), 2)
        cv2.drawContours(cut_image_test, [tmp // 4], -1, (0,255,255), 2)
