...
Y_MIN = 350
Y_MAX = 430
roi_poligon = np.array([[(X_MID-300, Y_MAX), (X_MID+300, Y_MAX), (X_MID+200, Y_MIN), (X_MID-200, Y_MIN)]])
...
while timer:
    ...
    lines = cv2.HoughLinesP(roi_image, 4, np.pi/180, 50,
                            np.array([]), minLineLength=10, maxLineGap=20)
    # Получение усреднённых вертикальных линий 
    averaged_lines = average_slope_intercept(color_image, lines)
    ...
    if averaged_lines is not None:
        ...
        cv2.putText(color_image, "No data", (5, 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
